...
function possibleSwap(arr){
    let flag = false;

    for(let i = 0;i<arr.length;i++){
        for(let k=0;k<arr.length;k++){
            let color = arr[i][k].color
            if(arr[i+1][k]){
                let temp = arr[i+1][k];
                arr[i+1][k] = arr[i][k];
                arr[i][k] = temp;
                ...
                if(arr[k-2]){
                    if(arr[i+1][k-2].color == color&& arr[i+1][k-1].color == color){
                        flag = true;
                    }
                }
                else if(arr[k+2]){
                    if(arr[i+1][k+1].color == color && arr[i+1][k+2].color == color){
                        flag = true;
                    }
                }

                else if(arr[k-1]){
                    if(arr[i+1][k-1].color == color && arr[i+1][k+1].color == color ){
                        flag = true;
                    }
                }
            }
